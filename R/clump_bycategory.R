...
#' @param directions (default 8) direction for clumping
#'
...
clump_bycategory = function(basemap, directions=8 ) {
  ...
  for (i in 1:length(ucats)) {
    ...
    tmp = terra::app(basemap, fun=function(x) { a = ifelse(x==ucats[i],1,0); return(a)})
    # tmp2 = raster::clump(tmp, directions=directions)
    tmp2 = terra::patches(tmp, directions=directions)
    lg = as.data.frame(terra::as.points(tmp2))
  ...
  res = terra::rasterize(lgall[,c("x","y")], basemap, lgall$clumps, fun=getmode)
